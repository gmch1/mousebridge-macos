...
MouseBridge 是一个采用 GPL-3.0-or-later 的轻量原生 macOS 输入工具底座，提供鼠标按键
映射、触控板多指快捷键、仅鼠标滚轮控制以及 Logitech HID++ 硬件 DPI 配置。它面向只
需要少量输入功能、不希望长期运行大型厂商套件的用户。
...
<p align="center">
  <img src="docs/images/settings-trackpad.png" alt="包含触控板多指快捷键设置的 MouseBridge 界面" width="640">
</p>
...
MouseBridge 并不打算复刻 Logitech Options+ 的全部功能。除专注的触控板多指绑定外，
目前不管理连续手势、按应用配置、Flow、固件升级、电量信息，也不管理 Logitech 摄像头
和键盘。
...
DPI 的读取和应用。滚动计算、配置校验、快捷键校验和 DPI 解码另有自动化测试。

以下路径已经实现，但尚未获得同等程度的实机覆盖：触控板多指绑定、水平滚动、滚轮中键
映射、Logitech Bolt 接收器、Intel Mac 和其他 Logitech 鼠标型号。项目最低部署目标为
macOS 13，但尚未逐个测试所有 macOS 版本。报告兼容性问题时，请附上产品名称、连接方式、
VID/PID、macOS 版本和诊断日志。

## 权限

拦截鼠标按键、修改滚轮事件和发送快捷键需要“辅助功能”权限；触控板绑定发送快捷键也
需要该权限。为了可靠地区分鼠标与触控板滚动，强烈建议同时开启“输入监控”；当前实测
环境已开启该权限。macOS 的隐私权限必须由用户手动批准，应用无法静默给自己授权。
...
标记为 **Pre-release** 的版本使用 ad-hoc 签名，Gatekeeper 可能要求手动批准。经过
Developer ID 签名和 Apple 公证的构建会发布为普通 Release。
...
构建脚本会优先使用 `/Applications/Xcode.app`，运行 XCTest 与二进制自检，构建 Release
可执行文件并组装应用包。默认的 ad-hoc 签名只用于贡献者本地构建。
...
推送 `v0.2.1` 这样的语义化版本 tag 后，流水线会校验 tag 与 `Info.plist` 版本是否一致，
...
- `AppConfig` 与 `ConfigStore`：带版本的 JSON、校验、原子写入与目录监控。
- `EventTapController`：按键拦截、输入源识别和滚轮转换。
- `MultitouchController` 与 `MultitouchGestureRecognizer`：原生私有框架设备回调，以及零
  数组分配、可独立测试的手势状态机；功能关闭或 MouseBridge 暂停时不会保留设备回调。
...
MouseBridge 的互操作能力与部分实现模式建立在以下项目的公开工作之上：

- [Scroll Reverser](https://github.com/pilotmoon/Scroll-Reverser)（Apache-2.0）：其公开
  实现帮助确认了触控板/鼠标的事件时序，以及可靠反转滚轮时同时更新 Quartz 和底层
  IOHID 滚动数据的必要性。
- [Solaar](https://github.com/pwr-Solaar/Solaar)（GPL-2.0-or-later）：其 HID++ 可调
  DPI 列表解码为 MouseBridge 的对应逻辑提供了实现依据。
- [TapBind](https://github.com/gmch1/TapBind)（衍生自
  [MiddleClick](https://github.com/artginzburg/MiddleClick)，GPL-3.0）：其
  MultitouchSupport 声明和交互行为为 MouseBridge 独立重写的可配置触控板状态机提供参考。

组合后的 MouseBridge 项目使用 GPL-3.0-or-later 分发；第三方来源部分继续保留原始声明
和许可证历史。详情见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) 和
[LICENSES](LICENSES/)。

## 兼容性与分发说明

为了可靠反转滚轮，MouseBridge 会动态加载 Scroll Reverser 也记录过的私有 IOHID 桥接
符号；触控板多指接触数使用 Apple 私有 MultitouchSupport 框架。滚轮私有符号不可用时会
退回公开 Quartz 字段；若 MultitouchSupport 发生不兼容变化，多指绑定将不可用。私有 API
可能随 macOS 更新而变化，因此新系统版本需要重新回归这两条链路。这也意味着项目更适合
直接分发，而不是 Mac App Store。

## 许可证

MouseBridge 是使用 [GPL-3.0-or-later](LICENSE) 分发的自由软件。对应源码可从本仓库及
每个 GitHub Release 获取，详见 [SOURCE.md](SOURCE.md)。
